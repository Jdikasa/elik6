...
class Document extends Component
{
    ...
    {
        $this->documents = $this->dossier->documents->where('statut_id', '!=', 6)->sortByDesc('created_at');
        ...
        if ($this->search) {
            ...
            {
                return Str::contains(Str::lower($document->libelle), Str::lower($this->search)) || Str::contains(Str::lower($document->reference), Str::lower($this->search));
            });
        }
        ...
        switch ($this->filter) {
            ...
            case 2:
                $this->filterText = 'A - Z';
                $this->documents = $this->documents->sortBy('libelle');
                break;
            case 3:
                $this->filterText = 'Z - A';
                $this->documents = $this->documents->sortByDesc('libelle');
                break;
            case 4:
