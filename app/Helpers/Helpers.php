...
if (!function_exists('imageOrlocal')) {
    function imageOrlocal($subject, $local = null, String $thumbnail = null)
    {
        if (!empty($subject) && $subject !== null) {
            if (Storage::disk('public')->exists($subject)) {
                return image($subject, $thumbnail);
...
if (!function_exists('files')) {
    ...
    {
        $files = '';
        if (!empty($file) && json_decode($file) != null && json_decode($file) != "") {
            # code...
            $files = json_decode($file);
        }
        // dd(!is_countable($files) && in_array(Str::lower(Str::afterLast($file,'.')), ['png','jpg', 'jpeg', 'webp']));
        $results = collect();
        ...
        if (is_null($files) || (is_countable($files) && (count($files) <= 0))) {
            ...
            $fichier->name = '';
            return $fichier;
        }elseif (!is_countable($files) && in_array(Str::lower(Str::afterLast($file,'.')), ['png','jpg', 'jpeg', 'webp']) ) {
            $fichier = new stdClass;
            $fichier->link = image($file);
            $fichier->name = Str::beforeLast($file,'.');
            return $fichier;
        }

        foreach ($files ?? [] as $file) {
            $fichier = new stdClass;
            $fichier->link = asset('storage') . '/' . $file->download_link;
...
if (!function_exists('get_file_size')) {
    function get_file_size($file_path, $precision = 2)
    {
        $fileSize = (int) File::size(storage_path('app/public/'.$file_path));
...
if (!function_exists('fileIcon')) {
    ...
    {
        if (!$file) {
            return asset('assets/svg/brands/google-docs-icon.svg');;
        }
        ...
        switch ($ext) {
            case 'docx':
                $icon = asset('assets/svg/brands/word-icon.svg');
                break;

            case 'pdf':
                $icon = asset('assets/svg/brands/pdf-icon.svg');
                break;

            case 'xlsx':
                $icon = asset('assets/svg/brands/excel-icon.svg');
                break;

            case 'pptx':
                $icon = asset('assets/svg/brands/google-slides-icon.svg');
                break;
            ...
            default:
                $icon = asset('assets/svg/brands/fichier-image.svg');
                break;
        }
...
if (!function_exists('convertionUnit')) {
    function convertionUnit($nombre)
    {
        if ($nombre == 0 || $nombre == null || $nombre == '') return '';
        $nombre = abs($nombre);
        $def = [
            [1, ''],
            [1000, 'k'],
        ...
        for ($i = 0; $i < count($def); $i++) {
            if ($nombre < $def[$i][0]) return $def[$i - 1][1];
        }
    }
}
