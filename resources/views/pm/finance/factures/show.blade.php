...
@section('body')
                ...
                <div class="card card-lg">
                    ...
                    <div class="card-body" id="to_print">
                            ...
                            <div class="col-md-5 align-self-md-start text-md-end">
                                ...
                                <div class="mt-3" style="font-size: 12px">
                                    <dl class="mb-0 d-flex justify-content-between align-items-center">
                                        ...
                                        <dd class="p-0 mb-0 border ms-1 col-md-auto border-bottom-0">
                                            <span class="p-1 text-center d-block"
                                                style="width: 110px">{{ $facture->id }}</span>
                                        </dd>
                                    </dl>
                                    ...
                                    <dl class="mb-0 d-flex justify-content-between align-items-center">
                                        ...
                                        <dd class="p-0 mb-0 border ms-1 col-md-auto border-bottom-0">
                                            <span class="p-1 text-center d-block"
                                                style="width: 110px">{{ $facture->created_at->format('d/m/Y') }}</span>
                                        </dd>
                                    </dl>
                                    ...
                                    <dl class="d-flex justify-content-between align-items-center">
                                        ...
                                        <dd class="p-0 mb-0 border ms-1 col-md-auto">
                                            <span class="p-1 text-center d-block" style="width: 110px">
                                                @if ($facture->date_limit_paie)
                                                    {{ $facture->date_limit_paie->format('d/m/Y') }}
                                                @else
                                                    ####
                                                @endif
                                            </span>
                        ...
                        <div class="mb-3 row">
                            <div class="col-md-6">
                                ...
                                <div class="mb-4">
                                    ...
                                    <small>
                                        {!! $facture->client->adresse->adresse_1 . ',<br>' . $facture->client->adresse->adresse_2 !!}
                                    </small>
                                </div>
                                <!-- End Form -->
                            </div>
                        ...
                        <div class="table-responsive js-add-fiel">
                            ...
                            @forelse ($facture->projects as $key => $project)
                                <!-- Content -->
                                <div class="mx-0 row flex-nowrap" style="width: 100%">
                                    <div class="py-2 border border-top-0 col-9 col-md-5">
                                        <small>
                                            {!! 'Equipement : '.$project->equipement->nom. '<br>Pays : ' .$project->certificat->country->name_fr !!}
                                        </small>
                                    </div>
                                    <!-- End Col -->
                                    ...
                                    <div class="py-2 col-7 col-sm-auto col-md-3 border-end border-bottom">
                                        <small>{{ $project->pivot->qt }}</small>
                                    </div>
                                    <!-- End Col -->

                                    <div class="py-2 col-4 col-md-2 border-end border-bottom">
                                        <small>{{ $project->pivot->prix * $project->pivot->qt }} $</small>
                                    </div>
                        ...
                        <div class="mb-3 row justify-content-md-end">
                                        ...
                                        <td class="">
                                            <small style="font-size: 11px">
                                                <ol class="m-0 p-0 ms-3">
                                                    <li>Le paiement total est attendu dans {{ $facture->created_at->diffInDays($facture->date_limit_paie) }} jours</li>
                                                    <li>Veillez inclure le numérro de la facture dans le check</li>
                                                    <li>Le paiement doit s'effectuer sur le numérro de compte : {{ $facture->compte?->num_compte }}</li>
                                                </ol>
                                            </small>
                                        </td>
                                    </tbody>
                            ...
                            @php
                                ...
                                $total_taxe = 0;
                                $total_net = 0;
                                $post_pay = $facture->transactions->sum('montant');
                                foreach ($facture->projects as $project) {
                                    $total += $project->pivot->prix * $project->pivot->qt;
                                    $taxe += 0;
                                    $total_taxe += $total + $taxe;
                                    $total_net = $total_taxe - $facture->transactions->sum('montant');
                                }
                            @endphp
    ...
    <div class="modal fade" id="email-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form class="modal-content" action="{{ route('pm.fin.facture.send') }}" method="POST" id="paiement-form">
                @csrf
    ...
    <div class="modal fade" id="paiement-modal" tabindex="-1" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-sm">
            <form class="modal-content" action="{{ route("pm.fin.finances.savePaiement") }}" method="POST" id="paiement-form">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Paiement</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            ...
            function Convert_HTML_To_PDF_and_Save() {
                ...
                doc.html(elementHTML, {
                    callback: function(doc) {
                        ...
                        $.ajax('{{ route('pm.factures.makeFile') }}', {
                            method: 'POST',
                            data: formData,
                            processData: false,
                            contentType: false,
